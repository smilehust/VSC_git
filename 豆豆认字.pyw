...
def revious_fun():
    global NOW_NUM, index, leftKey_flag, rightKey_flag, spaceKey_flag 
    ...
    if 0 <= index <= NOW_NUM - 1:
        ...
        pinyin_label['text'] = ' '.join(''.join(i)
                                        for i in pinyin(study[index]))
        yihui_button['text'] = '已会\n{}/{}'.format(index+1, NOW_NUM)
        if index == 0:
...
def yihui_fun():
    global NOW_NUM, index, leftKey_flag, rightKey_flag, spaceKey_flag, upKey_flag
...
def pinyin_fun():
    ...
    pinyin_label['text'] = ' '.join(''.join(i)
                                    for i in pinyin(shuru_entry.get()))
...
pinyin_button = tk.Button(root, height=2, width=20,
                          text='显示拼音', command=pinyin_fun)
...
yihui_button = tk.Button(root, height=5, width=20,
                         text='已会', command=yihui_fun)
yihui_button.pack(side='left')
next_button = tk.Button(root, height=5, width=30,
                        text='前进--->', command=next_fun)
next_button.pack(padx=108, side='left')

root.bind('<Left>', root_left)
root.bind('<Right>', root_right)
root.bind('<space>', root_space)
